...
class pandaEnv:
    ...
    def throw(self, pos):
        # increase joint 6 (impulse)
        # not that simple. simple release -> bare fall down

        self.apply_action_head_vertical(action=[pos])
        for _ in range(25):
            p.stepSimulation()
        # release grip
        self.apply_action_fingers([0.04, 0.04])
    ...
    def apply_action_head_vertical(self, action):
        ...
        idx_head = [self._joint_name_to_ids["panda_joint6"]]
