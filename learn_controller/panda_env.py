...
class pandaEnv:
    ...
    def get_joint_pos(self, joint_names=None, joint_ids=None):
        if joint_names is None and joint_ids is None:
            raise ValueError("Must specify joint names or joint ids")
        if joint_ids is None:
            joint_ids = [self._joint_name_to_ids[name] for name in joint_names]
        jointStates = p.getJointStates(
            self.robot_id,
            joint_ids,
            physicsClientId=self._physics_client_id,
    ...
    def throw(self, pos):
        ...
        self.apply_action_head(action=[panda_joint6, panda_joint3])
        for _ in range(25):
            p.stepSimulation()
        # release grip
        self.apply_action_fingers([0.04, 0.04])
    ...
    def apply_action_head(self, action):
        # move finger joints in position control
        assert len(action) == 2, f"Wrong number of actions! {action}"

        idx_head = [
            self._joint_name_to_ids["panda_joint6"],
            self._joint_name_to_ids["panda_joint3"],
        ]
        for i, idx in enumerate(idx_head):
            p.setJointMotorControl2(
                self.robot_id,
                idx,
                p.POSITION_CONTROL,
